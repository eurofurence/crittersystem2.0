 ...
 *
 * A template can only ask for the signed-in user's own topics, or for one of the shared staff topics.
 * Naming a topic in markup grants nothing in any case: the subscriber token decides what the hub will
 * deliver (see {@see \App\Mercure\TopicBuilder}). This exists so the vocabulary stays in one place,
...
final class MercureTopicExtension extends AbstractExtension
{
     ...
     *
     * The test is the gate's own redirect decision, never "has completed onboarding": administrators
     * are exempt from the gate, so the weaker test hides the navbar bell and the status widget from
     * every administrator and, with the hub URL gated the same way, stops their browser connecting at
     * all, taking chat, the planner and every other live surface down silently. The only users who
     * should not get live regions are the ones the gate really does redirect, because the fragments
     * those regions fetch are refused for them.
     *
